...
When a robot session is initialized, the framework automatically publishes the connector's identity as a key-value pair under the key `connector_type`. The value comes from the `CONNECTOR_TYPE` class variable on the `ConnectorSpecificConfig` subclass — the source of truth — and is guaranteed by `ConnectorRootConfig` to match the `connector_type` declared in YAML. This happens once per robot for both `Connector` and `FleetConnector` with no action required from the connector implementation.
