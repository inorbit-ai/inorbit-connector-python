...
The `robot-connector/` and `fleet-connector/` examples both ship with a small `metrics.py` module that demonstrates how concrete connectors declare their own Prometheus instruments using the `inorbit_edge.metrics.get_meter` and `with_counter_metric` helpers. Both `example.yaml` and `example.fleet.yaml` include a `metrics:` block (ports 9090 and 9091 respectively) so the connectors expose `/metrics` when run.
...
You'll see the framework signals (`inorbit_connector_up`, `..._execution_loop_ticks_total`, `..._execution_loop_errors_total`), the SDK's per-robot publish counters (`calls_publish_pose_total{robot_id="..."}`, etc.), and the connector-specific counters from `metrics.py` (`example_bot_api_requests_total{endpoint="..."}`).
